...
public class ActivityLogin extends ActivityBaseToDisplay {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			user.setText(PreferenceConnector.readString(ActivityLogin.this, PreferenceConnector.USER, null));
			pass.setText(PreferenceConnector.readString(ActivityLogin.this, PreferenceConnector.PASS, null));
		} catch (Exception e) {
		...
		login.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (userName.length() == 0 || passWorrd.length() == 0) {
					...
					PreferenceConnector.writeString(ActivityLogin.this, PreferenceConnector.USER, userName);
					PreferenceConnector.writeString(ActivityLogin.this, PreferenceConnector.PASS, passWorrd);
		...
		if (task.equals(loginTask)) {
			try {
				...
				if (code.toString().equals("0")) {
					...
					final String OTP = ((SoapPrimitive) THE_OTP.getProperty("THE_OTP")).toString();
				...
				Util.showAlert(context,
						"Có lỗi xảy ra khi kết nối đến hệ thống! Vui lòng thử lại hoặc báo với quản trị.");
			}
